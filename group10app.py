...
def generate_written_report(counts, total, dominant):
    ...
    report_lines = [
        ...
        ("Summary: Based on " + str(total) + " player " + comment_word + " collected from the gaming community, "
         "overall sentiment is " + tone + ". The dominant category is " + dominant
         + ", accounting for " + str(dominant_pct) + "% of all feedback."),
        "",
...
@st.cache_resource(show_spinner="Loading sentiment model... (first run may take 1-2 min)")
...
@st.cache_resource(show_spinner="Loading TTS model... (first run may take 1-2 min)")
def load_tts_model():
    if "HF_TOKEN" in st.secrets:
        login(token=st.secrets["HF_TOKEN"])
...
if run_button:
    ...
    report_html = written_report.replace(chr(10), "<br>")
